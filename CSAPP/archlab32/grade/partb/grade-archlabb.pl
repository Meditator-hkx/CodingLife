...
sub usage {
    ...
    printf STDERR "  -s <codedir> Directory where Y86 simulators are located\n";
    die "\n";
}
...
# 
# Make sure the Y86 tools directory and the sssim binary exists
#
...
#
# Run synthetic regression tests for iaddl
#
print "\n";
print "**********\n";
print "Part 3: Extensive regression tests for iaddl\n";
print "**********\n";
system("(cd $tmpdir/sim/ptest; make SIM=../seq/ssim TFLAGS=-i  > outfile 2>&1)") == 0
  or die "ERROR: Could not run extensive regression tests for iaddl\n";
...
if ($succeed == $total) {
    $partb_scores[1] = $IADDL_POINTS;
}    
print "Score: $partb_scores[1]/$IADDL_POINTS\n";
