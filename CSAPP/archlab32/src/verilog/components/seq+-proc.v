...
// PC stage
  ...
  cenrreg #(1) pb(pCnd, Cnd, running & stat == SAOK,
        resetting, 0, clock);  // Previous Cnd
  cenrreg #(32) pvm(pValM, valM, running & stat == SAOK,
        resetting, 0, clock);  // Previous valM
  cenrreg #(32) pvc(pValC, valC, running & stat == SAOK,
        resetting, 0, clock);  // Previous valC
  cenrreg #(32) pvp(pValP, valP, running & stat == SAOK,
        resetting, 0, clock);  // Previous valP

// Fetch stage

  split splt(instr[7:0], imem_icode, imem_ifun);
  align aln(instr[47:8], need_regids, rA, rB, valC);
  pc_increment pci(pc, need_regids, need_valC, valP);

// Decode stage
  regfile regf(dstE, valE, dstM, valM, srcA, valA, srcB, valB, 
     resetting, clock,
...
// Operational control
   ...
   always
	...
	begin
	   ...
	   if (mem_write)
	     begin
		$display("Writing 0x%x to 0x%x", mem_data, mem_addr);
	     end
	end
